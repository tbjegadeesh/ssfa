...
import { userEndpoints } from '../../../common/Services/Auth/userEndPoints';
...
const SignIn: React.FC = () => {
  ...
  // Function to handle the API call using axios
  const loginUser: any = async (formData: LoginData): Promise<any> => {
    try {
      const response = await axios.post(userEndpoints.login(), formData);
      return response.data; // Return the actual response data
    } catch (error: any) {
      // Check if it's an Axios error and contains a response
      if (error.response) {
        // You can throw a new error with the message from the response
        throw new Error(error.response.data.message || null);
      } else {
        // Handle any other error cases (network issues, etc.)
        throw new Error(error.message || null);
      }
    }
  };

  // Specify the types for useMutation
  const mutations = useMutation<
    LoginData,
    Error,
    { url: string; formData: LoginData }
  >({
    // mutationFn: loginUser,
    mutationFn: ({ url, formData }) => loginUser(formData),
    onSuccess: (data: any) => {
      if (data.success) {
  ...
  const handleSubmit = (e: React.FormEvent<HTMLFormElement>) => {
    ...
    console.log('Form submitted dwith:', formData);
    // You can also implement actual sign-in logic here
    const url = '/api/login';
    mutations.mutate({ url, formData });
  };
  ...
  // // Second useQuery to fetch posts
  // const {
  //   data: posts,
  ...
  //   ["posts", users?.[0]?.id],  // Depend on users data
  //   () => fetchPosts(users?.[0]?.id),
  //   { enabled: !!users }  // Only run if users are fetched
  // );

  // const [userId, setUserId] = useState(1); // Initial userId

  // // Fetch user data based on the userId
  // const { data, isLoading, error } = useQuery({
  //   queryKey: ["user", userId],  // userId is a dependency for the query
  //   queryFn: () => fetchUser(userId), // Pass userId as an argument to the query function
  // });
    ...
    <>
      ...
      <div className="rounded-sm border border-stroke bg-white shadow-default dark:border-strokedark dark:bg-boxdark">
        <div className="flex flex-wrap">
          <div className="w-full border-stroke dark:border-strokedark xl:w-1/2 xl:border-l-2 signin-bg flex items-center">
            <div className="w-full p-4 sm:p-12.5 xl:p-17.5">
              <div className="py-17.5 px-26 text-center">
                <Link className="mb-5.5 inline-block" to="/">
                  <img className="hidden dark:block" src={Logo} alt="Logo" />
                  <img className="light:hidden" src={Logo} alt="Logo" />
                </Link>

                <p className="2xl:px-20 text-slate-300">
              ...
              <form onSubmit={handleSubmit}>
                <div className="mb-4">
                  ...
                  <div className="relative">
                    <input
                      type="email"
                      name="email" // Add name attribute for generic handler
                      placeholder="Enter your email"
                      value={formData.email}
                      onChange={handleChange} // Use generic change handler
                ...
                <div className="mb-6">
                  ...
                  <div className="relative">
                    <input
                      type="password"
                      name="password" // Add name attribute for generic handler
